...
def PIM.Map2 {n : Nat} {α : Type} (f : α -> α -> α) :
  let PO := PimObj α n
  PO -> PO -> PO :=
...
def PIM.Fold {n : Nat} {α : Type} (acc : α -> α -> α) :
  α -> PimObj α n -> α :=
  fun (init : α) =>
    fun (src : PimObj α n) =>
      Vector.foldl (acc) (init) src.elems

def pimAdd {α : Type} [Add α] {n : Nat}
  (src1 src2 : PimObj α n) : PimObj α n
  := PIM.Map2 (· + ·) src1 src2

def pimSub {α : Type} [Sub α] {n : Nat} (src1 src2 : PimObj (α := α) n) : PimObj (α := α) n :=
  let elems := src1.elems.zipWith (· - ·) src2.elems
  PimObj.mk elems

def pimRedSum {α : Type} {n : Nat} [Zero α] [Add α] :
  PimObj α n -> α :=
  fun (src : PimObj α n) =>
    PIM.Fold (fun a b => a + b) 0 src

def pimRedSub {α : Type} {n : Nat} [Zero α] [Sub α] :
  PimObj α n -> α :=
  fun (src : PimObj α n) =>
    PIM.Fold (fun a b => a - b) 0 src

def pimMul {α : Type} [Mul α] {n : Nat} :
  PimObj α n -> PimObj α n -> PimObj α n :=
  fun (src1 src2 : PimObj α n) => PIM.Map2 (fun a b => a * b) src1 src2

def pimMAC {α : Type} [Mul α] [Zero α] [Add α] {n : Nat} :
